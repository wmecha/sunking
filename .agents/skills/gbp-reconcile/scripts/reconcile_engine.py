...
Master Tracker column indices (A=0 .. AA=26, since the client added V "Images?" and
W "Images link(s)" in 2026-07): 0 store_code, 1 business_name, 2 country,
3 location_type, 5 claiming_issue(F), 8 ov(I), 10 ou(K), 11 duplicate(L),
12/13 phil lat/lng, 14/15 storeloc lat/lng, 17 address(R), 18 phone(S), 19 locality(T),
20 tracker_status(U), 21 images?(V), 22 images_links(W), 23 maps_link(X),
24/25 maps lat/lng (Y/Z), 26 initial_claim_status(AA).
"""
...
def _run():
    ...
    r, _ = run_composio_tool(tool_slug="GOOGLESHEETS_BATCH_GET", account="sunking-master-tracker",
        arguments={"spreadsheet_id": SS, "ranges": ["'Master Tracker'!A5:AA1000"],
    ...
    for row in rows:
        ...
        targets[code] = dict(
            ...
            address=s(c(row, 17)), city=s(c(row, 19)), google_maps_url=s(c(row, 23)),
            primary_phone=s(c(row, 18)), latitude=nf(c(row, 12), c(row, 24), c(row, 14)),
            longitude=nf(c(row, 13), c(row, 25), c(row, 15)),
